...
import numpy as np
import Utils.readsnap as readsnap
import sys,os,h5py
...
def read_gadget_header( ifile):
    '''
    Routine to read the gadget2 header. It prints a few values read
    from the header as a cross-check against input values in the params
    dictionary.
    '''
    # input files
    snapshot = ifile
    # read header
    g_header   = header(snapshot)
    BoxSize  = g_header.boxsize#/1e3  #Mpc/h
    Nall     = g_header.nall[1]         #Total number of particles
    Omega_m  = g_header.omega_m      #value of Omega_m
    Omega_l  = g_header.omega_l      #value of Omega_l
    redshift = g_header.redshift     #redshift of the snapshot
    NumFile  = g_header.filenum      #number of files

    print('Gadget CDM particles\n')
    print(' Total number of files: %d' %NumFile)
    print(' Total number of particles: %d\n' %Nall)
    print(' Checking on user input...\n')
    
    print('Box size read from Gadget header: %f'%BoxSize)
    print('Redshift read from Gadget header: %f'%redshift)
    print('Omega_m read from Gadget header: %f'%Omega_m)
    return Nall, NumFile
